...
let
  ...
  gettyCmd = extraArgs: "@${pkgs.util-linux_full}/sbin/agetty agetty --login-program ${pkgs.shadow}/bin/login ${autologinArg} ${extraArgs}";
...

{
  ...
  config = {
    ...
    services.mingetty.greetingLine = mkDefault ''<<< Welcome to Triton ${config.system.nixosLabel} (\m) - \l >>>'';
