...

{
  ...
  config = mkMerge [
    (mkIf config.systemd.coredump.enable {
      ...
      systemd.additionalUpstreamUnits = [ "systemd-coredump.socket" "systemd-coredump@.service" ];
      ...
      boot.kernel.sysctl."kernel.core_pattern" = "|${config.systemd.package}/lib/systemd/systemd-coredump %P %u %g %s %t %c %e";
    })
