...
let
  recursivePthLoader = import ../recursive-pth-loader/default.nix {
    stdenv = stdenv;
    python = python;
  };
  env = (
    ...
    let
      paths = stdenv.lib.filter (x : x ? pythonPath) (
        stdenv.lib.closePropagation extraLibs
      ) ++ [ python recursivePthLoader ];
    in
