...
rec {
  ...
  overrideInStdenv = stdenv: pkgs: stdenv.override (prev: {
  ...
  #   };
  overrideSetup = stdenv: setupScript: stdenv.override {
    inherit setupScript;
  };
  ...
  makeStaticBinaries = stdenv: stdenv // {
    mkDerivation = args: stdenv.mkDerivation (args // {
      ...
      configureFlags = toString args.configureFlags or ""
        + " --disable-shared"; # brrr...
    });
    isStatic = true;
  };
  ...
  makeStdenvCross = stdenv: cross: binutilsCross: gccCross: stdenv // {
    ...
    let
      ...
      nativeInputsFromBuildInputs =
        stdenv.lib.filter hostAsNativeDrv buildInputsNotNull;
